#!/bin/bash

cp ./local/punchswap/contracts_local.sh ./solidity/lib/punch-swap-v3-contracts/
cp ./local/punchswap/flow-emulator.json ./solidity/lib/punch-swap-v3-contracts/script/deployParameters/

cp ./local/punchswap/punchswap.env ./solidity/lib/punch-swap-v3-contracts/.env
# cp ./local/punchswap/punchswap.env ./solidity/lib/punch-swap-core-contracts/.env

# deployer
flow transactions send ./cadence/transactions/mocks/transfer_to_evm.cdc 0xC31A5268a1d311d992D637E8cE925bfdcCEB4310 1000.0

# 
#flow transactions send ./cadence/transactions/mocks/amm/transfer_to_evm.cdc 0x4e59b44847b379578588920cA78FbF26c0B4956C 1000.0
# fund CREATE2 deployer
flow transactions send ./cadence/transactions/mocks/transfer_to_evm.cdc 0x3fab184622dc19b6109349b94811493bf2a45362 1000.0
# Deploy CREATE2
cast rpc eth_sendRawTransaction  --rpc-url http://localhost:8545

# cd ./solidity/lib/punch-swap-core-contracts

# forge script ./script/WrappedFlowDeploy.s.sol --broadcast --rpc-url http://localhost:8545 
# forge script ./script/Multicall3Deploy.s.sol --broadcast --rpc-url http://localhost:8545 
# forge script ./script/PunchSwapV2FactoryDeploy.s.sol --broadcast --rpc-url http://localhost:8545 

cd ./solidity/lib/punch-swap-v3-contracts/

./contracts_local.sh

