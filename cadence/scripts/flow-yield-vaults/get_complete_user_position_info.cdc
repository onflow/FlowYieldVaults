...
fun main(address: Address): CompleteUserSummary {
    let yieldVaultManager = getAccount(address).capabilities.borrow<&FlowYieldVaults.YieldVaultManager>(FlowYieldVaults.YieldVaultManagerPublicPath)
    ...
    // Note: FlowALPv1 positions and FlowYieldVaults yield vaults use different ID systems
    // We'll calculate health manually since yield vault IDs don't correspond to FlowALPv1 position IDs
    ...
    for yieldVaultId in yieldVaultIds {
        if let yieldVault = yieldVaultManager!.borrowYieldVault(id: yieldVaultId) {
            ...
            // This bypasses the FlowALPv1 overflow issue
            ...
            // FlowALPv1 positions use sequential IDs (0, 1, 2, ...) while yield vault IDs are different
