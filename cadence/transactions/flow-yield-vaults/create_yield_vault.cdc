...
///     FlowYieldVaults. See `FlowYieldVaults.getSupportedStrategies()` to get those currently supported.
...
transaction(strategyIdentifier: String, vaultIdentifier: String, amount: UFix64) {
    ...
    prepare(signer: auth(BorrowValue, SaveValue, StorageCapabilities, PublishCapability, CopyValue) &Account) {
        ...
        if signer.storage.type(at: FlowYieldVaults.YieldVaultManagerStoragePath) == nil {
            signer.storage.save(<-FlowYieldVaults.createYieldVaultManager(betaRef: self.betaRef), to: FlowYieldVaults.YieldVaultManagerStoragePath)
