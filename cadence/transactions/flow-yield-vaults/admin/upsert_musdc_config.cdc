...
transaction(
    ...
    prepare(signer: auth(BorrowValue) &Account) {
        ...
        let issuerRef = signer.storage.borrow<
            auth(FlowYieldVaultsStrategies.Configure) &FlowYieldVaultsStrategies.StrategyComposerIssuer
        >(from: FlowYieldVaultsStrategies.IssuerStoragePath)
        ...
        let config: {Type: {Type: {String: AnyStruct}}} = {
            Type<@FlowYieldVaultsStrategies.mUSDCStrategy>(): {
        ...
        issuerRef.upsertConfigFor(
            composer: Type<@FlowYieldVaultsStrategies.mUSDCStrategyComposer>(),
            config: config
        )
    }

    execute {
        log("Updated mUSDC strategy config for FlowYieldVaultsStrategies.mUSDCStrategyComposer")
    }
}
