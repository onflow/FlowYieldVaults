...
// Reuse the entire body of the legacy Scenario 3A test
// to avoid duplication we'll import the original code logic as a function.

access(all)
fun test_RebalanceTideScenario3A_V2() {
    // The content is identical to legacy test body; we include via relative import is not possible, so duplicating minimal call.
    // For brevity, we'll call the legacy logic here directly since data paths & helpers are shared.
    ...
    Test.assert(equalAmounts(a: yieldTokensBefore, b: expectedYieldTokenValues[0], tolerance: 0.01), message: "Mismatch before flow price decrease")
    ...
    let yieldAfterFlow = getAutoBalancerBalanceV2(id: tideIDs[0])!
    Test.assert(equalAmounts(a: yieldAfterFlow, b: expectedYieldTokenValues[1], tolerance: 0.01), message: "Mismatch after flow price decrease")

    setMockOraclePrice(signer: tidalYieldAccount, forTokenIdentifier: yieldTokenIdentifier, price: yieldPriceIncrease)

    rebalanceTideV2(signer: tidalYieldAccount, id: tideIDs[0], force: true, beFailed: false)

    let yieldAfterIncrease = getAutoBalancerBalanceV2(id: tideIDs[0])!
    Test.assert(equalAmounts(a: yieldAfterIncrease, b: expectedYieldTokenValues[2], tolerance: 0.01), message: "Mismatch after yield price increase")
