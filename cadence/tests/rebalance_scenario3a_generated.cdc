...
access(all) fun test_Scenario4_Path_A() {
    ...
    while i < 3 {
        log("Step \(i)")
        setMockOraclePrice(signer: tidalYieldAccount, forTokenIdentifier: flowTokenIdentifier, price: [1.0, 0.8, 0.8][i])
        setMockOraclePrice(signer: tidalYieldAccount, forTokenIdentifier: yieldTokenIdentifier, price: [1.0, 1.0, 1.2][i])
        rebalanceTide(signer: tidalYieldAccount, id: tideIDs[0], force: true, beFailed: false)
        rebalancePosition(signer: protocolAccount, pid: pid, force: true, beFailed: false)
        let actualDebt = getMOETDebtFromPosition(pid: pid)
        let actualYield = getYieldUnits(id: tideIDs[0])
        let actualCollateral = getFlowCollateralValue(pid: pid, flowPrice: [1.0, 0.8, 0.8][i])
        log("Step \(i): Expected Debt [615.384615385, 492.307692308, 552.899408284][i], Actual \(actualDebt)")
        Test.assert(equalAmounts(a: actualDebt, b: [615.384615385, 492.307692308, 552.899408284][i], tolerance: 0.00000001), message: "Debt mismatch at step \(i)")
        Test.assert(equalAmounts(a: actualYield, b: [615.384615385, 492.307692308, 460.749506903][i], tolerance: 0.00000001), message: "Yield mismatch at step \(i)")
        Test.assert(equalAmounts(a: actualCollateral, b: [1000.0, 800.0, 898.461538462][i], tolerance: 0.00000001), message: "Collateral mismatch at step \(i)")
        i = i + 1
    }}
