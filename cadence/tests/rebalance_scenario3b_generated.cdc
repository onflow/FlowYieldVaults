...
access(all) fun test_Scenario4_Path_B() {
    ...
    while i < 3 {
        log("Step \(i)")
        setMockOraclePrice(signer: tidalYieldAccount, forTokenIdentifier: flowTokenIdentifier, price: [1.0, 1.5, 1.5][i])
        setMockOraclePrice(signer: tidalYieldAccount, forTokenIdentifier: yieldTokenIdentifier, price: [1.0, 1.0, 1.3][i])
        rebalanceTide(signer: tidalYieldAccount, id: tideIDs[0], force: true, beFailed: false)
        rebalancePosition(signer: protocolAccount, pid: pid, force: true, beFailed: false)
        let actualDebt = getMOETDebtFromPosition(pid: pid)
        let actualYield = getYieldUnits(id: tideIDs[0])
        let actualCollateral = getFlowCollateralValue(pid: pid, flowPrice: [1.0, 1.5, 1.5][i])
        log("Step \(i): Expected Debt [615.384615385, 923.076923077, 1093.49112426][i], Actual \(actualDebt)")
        Test.assert(equalAmounts(a: actualDebt, b: [615.384615385, 923.076923077, 1093.49112426][i], tolerance: 0.00000001), message: "Debt mismatch at step \(i)")
        Test.assert(equalAmounts(a: actualYield, b: [615.384615385, 923.076923077, 841.147018662][i], tolerance: 0.00000001), message: "Yield mismatch at step \(i)")
        Test.assert(equalAmounts(a: actualCollateral, b: [1000.0, 1500.0, 1776.923076923][i], tolerance: 0.00000001), message: "Collateral mismatch at step \(i)")
        i = i + 1
    }}
