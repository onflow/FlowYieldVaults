...
fun setup() {
	...
	// add YieldToken & AutoBalancer strategy to TidalYield
	addStrategyComposer(
...
access(all)
fun test_RebalanceTideScenario1_V2() {
	...
	for flowPrice in flowPrices {
		...
		log("[V2 TEST] Tide balance before flow price \(flowPrice) \(tideBalance ?? 0.0)")

		setMockOraclePrice(signer: tidalYieldAccount, forTokenIdentifier: flowTokenIdentifier, price: flowPrice)

		tideBalance = getTideBalance(address: user.address, tideID: tideIDs![0])

		log("[V2 TEST] Tide balance before flow price \(flowPrice) rebalance: \(tideBalance ?? 0.0)")

		// Get yield token balance before rebalance
		let yieldTokensBefore = getAutoBalancerBalanceV2(id: tideIDs![0]) ?? 0.0
		let currentValueBefore = getAutoBalancerCurrentValue(id: tideIDs![0]) ?? 0.0
		
		rebalanceTideV2(signer: tidalYieldAccount, id: tideIDs![0], force: false, beFailed: false)
	rebalancePosition(signer: protocolAccount, pid: pid, force: false, beFailed: false)

		tideBalance = getTideBalance(address: user.address, tideID: tideIDs![0])

		log("[V2 TEST] Tide balance after flow before \(flowPrice): \(tideBalance ?? 0.0)")
		
		// Get yield token balance after rebalance
		let yieldTokensAfter = getAutoBalancerBalanceV2(id: tideIDs![0]) ?? 0.0
		...
		log("\n=== V2 SCENARIO 1 DETAILS for Flow Price \(flowPrice) ===")
