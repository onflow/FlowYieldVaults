...
fun setup() {
	...
	// enable mocked Strategy creation - V2 version
	addStrategyComposer(
...
access(all)
fun test_RebalanceTideScenario2_V2() {
	...
	for index, yieldTokenPrice in yieldPriceIncreases {
		...
		log("[V2 TEST] Tide balance before yield price \(yieldTokenPrice): \(tideBalance ?? 0.0)")

		setMockOraclePrice(signer: tidalYieldAccount, forTokenIdentifier: yieldTokenIdentifier, price: yieldTokenPrice)

		tideBalance = getTideBalance(address: user.address, tideID: tideIDs![0])

		log("[V2 TEST] Tide balance before yield price \(yieldTokenPrice) rebalance: \(tideBalance ?? 0.0)")
	...
	rebalancePosition(signer: protocolAccount, pid: pid, force: false, beFailed: false)
		...
		log("[V2 TEST] Tide balance after yield before \(yieldTokenPrice) rebalance: \(tideBalance ?? 0.0)")
		...
		log("\n=== V2 PRECISION COMPARISON for Yield Price \(yieldTokenPrice) ===")
