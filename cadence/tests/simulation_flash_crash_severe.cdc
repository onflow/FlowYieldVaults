...
import "simulation_flash_crash_severe_helpers.cdc"
...
fun setup() {
    ...
    let ytPool = simulation_flash_crash_severe_pools["moet_yt"]!
    ...
    let btcPool = simulation_flash_crash_severe_pools["moet_btc"]!
    let initialBtcPrice = simulation_flash_crash_severe_prices[0]
    setPoolToPriceWithTVL(
        factoryAddress: factoryAddress,
...
access(all) fun runSimulation(config: SimConfig, label: String): SimResult {
    ...
    log("Notes: 25% BTC crash over 5 min, floor for 20 min, exponential recovery")
...
access(all)
fun test_FlashCrashSevere_ZeroLiquidations() {
    let result = runSimulation(
        config: SimConfig(
            prices: simulation_flash_crash_severe_prices,
            tickIntervalSeconds: 60.0, // 1 min per tick, matching Python sim
            numAgents: 10,
            fundingPerAgent: 1.0,
            yieldAPR: simulation_flash_crash_severe_constants.yieldAPR,
            ...
            initialHF: 1.15,
            ytPoolTVL: simulation_flash_crash_severe_pools["moet_yt"]!.size,
            ytPoolConcentration: simulation_flash_crash_severe_pools["moet_yt"]!.concentration
        ),
        label: "FlashCrashSevere"
    )
    ...
    log("=== TEST PASSED: Zero liquidations under flash crash (severe) ===")
}
