...
///
access(all) contract FlowYieldVaultsStrategies {
    ...
    access(all) resource TracerStrategyComposer : FlowYieldVaults.StrategyComposer {
        ...
        access(all) fun createStrategy(
            ...
            let recurringConfig = FlowYieldVaultsStrategies._createRecurringConfig(withID: uniqueID)
            ...
            let poolCap = FlowYieldVaultsStrategies.account.storage.load<Capability<auth(FlowCreditMarket.EParticipant, FlowCreditMarket.EPosition) &FlowCreditMarket.Pool>>(
    ...
    access(all) resource mUSDCStrategy : FlowYieldVaults.Strategy, DeFiActions.IdentifiableResource {
        /// An optional identifier allowing protocols to identify stacked connector operations by defining a protocol-
        /// specific Identifier to associated connectors on construction
        access(contract) var uniqueID: DeFiActions.UniqueIdentifier?
        access(self) let position: FlowCreditMarket.Position
    ...
    access(all) resource mUSDCStrategyComposer : FlowYieldVaults.StrategyComposer {
        ...
        access(all) fun createStrategy(
            ...
            let moetToYieldAMMSwapper = UniswapV3SwapConnectors.Swapper(
                    ...
                    inVault: moetTokenType,
                    outVault: yieldTokenType,
                    coaCapability: FlowYieldVaultsStrategies._getCOACapability(),
                    uniqueID: uniqueID
                )
            ...
            let moetToUnderlyingAssetSwapper = UniswapV3SwapConnectors.Swapper(
                    ...
                    feePath: [100],
                    inVault: moetTokenType,
                    outVault: underlying4626AssetType,
                    coaCapability: FlowYieldVaultsStrategies._getCOACapability(),
                    uniqueID: uniqueID
                )
            ...
            let underlyingTo4626Swapper = ERC4626SwapConnectors.Swapper(
                    asset: underlying4626AssetType,
                    vault: yieldTokenEVMAddress,
                    coa: FlowYieldVaultsStrategies._getCOACapability(),
                    feeSource: FlowYieldVaultsStrategies._createFeeSource(withID: uniqueID),
                    uniqueID: uniqueID
                )
            ...
            // open a FlowCreditMarket position
            let poolCap = FlowYieldVaultsStrategies.account.storage.copy<Capability<auth(FlowCreditMarket.EParticipant, FlowCreditMarket.EPosition) &FlowCreditMarket.Pool>>(
                    from: FlowCreditMarket.PoolCapStoragePath
    ...
    fun _createRecurringConfig(withID: DeFiActions.UniqueIdentifier?): DeFiActions.AutoBalancerRecurringConfig {
        ...
        return DeFiActions.AutoBalancerRecurringConfig(
            interval: 60 * 60 * 30,  // Rebalance every 30 minutes
