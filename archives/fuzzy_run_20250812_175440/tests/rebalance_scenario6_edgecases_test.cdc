...
access(all)
fun test_RebalanceTideScenario6_EdgeCases_VeryLowFlow() {
    let fundingAmount = 1000.0
    let user = Test.createAccount()

    let flowPrices = [0.01000000]
    let yieldPrices = [1.00000000]
    let expectedDebts = [6.15384615]
    let expectedYieldUnits = [6.15384615]
    let expectedCollaterals = [10.00000000]
    let actions: [String] = ["Repay 609.230769231"]
    ...
    logStep("Scenario6_EdgeCases_VeryLowFlow", 0, actualDebt, expectedDebts[0], actualYieldUnits, expectedYieldUnits[0], actualCollateral, expectedCollaterals[0])
    ...
    while i < flowPrices.length {
        ...
        logStep("Scenario6_EdgeCases_VeryLowFlow", i, actualDebt, expectedDebts[i], actualYieldUnits, expectedYieldUnits[i], actualCollateral, expectedCollaterals[i])
...
access(all)
fun test_RebalanceTideScenario6_EdgeCases_VeryHighFlow() {
    let fundingAmount = 1000.0
    let user = Test.createAccount()

    let flowPrices = [100.00000000]
    let yieldPrices = [1.00000000]
    let expectedDebts = [61538.46153846]
    let expectedYieldUnits = [61538.46153846]
    let expectedCollaterals = [100000.00000000]
    let actions: [String] = ["Borrow 60923.076923077"]
    ...
    logStep("Scenario6_EdgeCases_VeryHighFlow", 0, actualDebt, expectedDebts[0], actualYieldUnits, expectedYieldUnits[0], actualCollateral, expectedCollaterals[0])
    let okDebt0 = equalAmounts(a: actualDebt, b: expectedDebts[0], tolerance: 0.0000001)
    let okY0 = equalAmounts(a: actualYieldUnits, b: expectedYieldUnits[0], tolerance: 0.0000001)
    let okC0 = equalAmounts(a: actualCollateral, b: expectedCollaterals[0], tolerance: 0.0000001)
    if !(okDebt0 && okY0 && okC0) { allGood = false }
    ...
    while i < flowPrices.length {
        ...
        logStep("Scenario6_EdgeCases_VeryHighFlow", i, actualDebt, expectedDebts[i], actualYieldUnits, expectedYieldUnits[i], actualCollateral, expectedCollaterals[i])
        let okDebt = equalAmounts(a: actualDebt, b: expectedDebts[i], tolerance: 0.0000001)
        let okY = equalAmounts(a: actualYieldUnits, b: expectedYieldUnits[i], tolerance: 0.0000001)
        let okC = equalAmounts(a: actualCollateral, b: expectedCollaterals[i], tolerance: 0.0000001)
        if !(okDebt && okY && okC) { allGood = false }
...
access(all)
fun test_RebalanceTideScenario6_EdgeCases_VeryHighYield() {
    let fundingAmount = 1000.0
    let user = Test.createAccount()

    let flowPrices = [1.00000000]
    let yieldPrices = [50.00000000]
    let expectedDebts = [19171.59763315]
    let expectedYieldUnits = [383.43195266]
    let expectedCollaterals = [31153.84615387]
    let actions: [String] = ["Bal sell 603.076923077 | Borrow 18556.213017763"]
    ...
    logStep("Scenario6_EdgeCases_VeryHighYield", 0, actualDebt, expectedDebts[0], actualYieldUnits, expectedYieldUnits[0], actualCollateral, expectedCollaterals[0])
    let okDebt0 = equalAmounts(a: actualDebt, b: expectedDebts[0], tolerance: 0.0000001)
    let okY0 = equalAmounts(a: actualYieldUnits, b: expectedYieldUnits[0], tolerance: 0.0000001)
    let okC0 = equalAmounts(a: actualCollateral, b: expectedCollaterals[0], tolerance: 0.0000001)
    if !(okDebt0 && okY0 && okC0) { allGood = false }
    ...
    while i < flowPrices.length {
        ...
        logStep("Scenario6_EdgeCases_VeryHighYield", i, actualDebt, expectedDebts[i], actualYieldUnits, expectedYieldUnits[i], actualCollateral, expectedCollaterals[i])
        let okDebt = equalAmounts(a: actualDebt, b: expectedDebts[i], tolerance: 0.0000001)
        let okY = equalAmounts(a: actualYieldUnits, b: expectedYieldUnits[i], tolerance: 0.0000001)
        let okC = equalAmounts(a: actualCollateral, b: expectedCollaterals[i], tolerance: 0.0000001)
        if !(okDebt && okY && okC) { allGood = false }
...
access(all)
fun test_RebalanceTideScenario6_EdgeCases_BothVeryLow() {
    let fundingAmount = 1000.0
    let user = Test.createAccount()

    let flowPrices = [0.05000000]
    let yieldPrices = [0.02000000]
    let expectedDebts = [30.76923077]
    let expectedYieldUnits = [-28615.38461542]
    let expectedCollaterals = [50.00000000]
    let actions: [String] = ["Repay 584.615384616"]
    ...
    logStep("Scenario6_EdgeCases_BothVeryLow", 0, actualDebt, expectedDebts[0], actualYieldUnits, expectedYieldUnits[0], actualCollateral, expectedCollaterals[0])
    let okDebt0 = equalAmounts(a: actualDebt, b: expectedDebts[0], tolerance: 0.0000001)
    let okY0 = equalAmounts(a: actualYieldUnits, b: expectedYieldUnits[0], tolerance: 0.0000001)
    let okC0 = equalAmounts(a: actualCollateral, b: expectedCollaterals[0], tolerance: 0.0000001)
    if !(okDebt0 && okY0 && okC0) { allGood = false }
    ...
    while i < flowPrices.length {
        ...
        logStep("Scenario6_EdgeCases_BothVeryLow", i, actualDebt, expectedDebts[i], actualYieldUnits, expectedYieldUnits[i], actualCollateral, expectedCollaterals[i])
        let okDebt = equalAmounts(a: actualDebt, b: expectedDebts[i], tolerance: 0.0000001)
        let okY = equalAmounts(a: actualYieldUnits, b: expectedYieldUnits[i], tolerance: 0.0000001)
        let okC = equalAmounts(a: actualCollateral, b: expectedCollaterals[i], tolerance: 0.0000001)
        if !(okDebt && okY && okC) { allGood = false }
...
access(all)
fun test_RebalanceTideScenario6_EdgeCases_MinimalPosition() {
    let fundingAmount = 1000.0
    let user = Test.createAccount()

    let flowPrices = [1.00000000]
    let yieldPrices = [1.00000000]
    let expectedDebts = [0.61538462]
    let expectedYieldUnits = [0.61538462]
    let expectedCollaterals = [1.00000000]
    let actions: [String] = ["none"]
    ...
    logStep("Scenario6_EdgeCases_MinimalPosition", 0, actualDebt, expectedDebts[0], actualYieldUnits, expectedYieldUnits[0], actualCollateral, expectedCollaterals[0])
    let okDebt0 = equalAmounts(a: actualDebt, b: expectedDebts[0], tolerance: 0.0000001)
    let okY0 = equalAmounts(a: actualYieldUnits, b: expectedYieldUnits[0], tolerance: 0.0000001)
    let okC0 = equalAmounts(a: actualCollateral, b: expectedCollaterals[0], tolerance: 0.0000001)
    if !(okDebt0 && okY0 && okC0) { allGood = false }
    ...
    while i < flowPrices.length {
        ...
        logStep("Scenario6_EdgeCases_MinimalPosition", i, actualDebt, expectedDebts[i], actualYieldUnits, expectedYieldUnits[i], actualCollateral, expectedCollaterals[i])
        let okDebt = equalAmounts(a: actualDebt, b: expectedDebts[i], tolerance: 0.0000001)
        let okY = equalAmounts(a: actualYieldUnits, b: expectedYieldUnits[i], tolerance: 0.0000001)
        let okC = equalAmounts(a: actualCollateral, b: expectedCollaterals[i], tolerance: 0.0000001)
        if !(okDebt && okY && okC) { allGood = false }
...
access(all)
fun test_RebalanceTideScenario6_EdgeCases_LargePosition() {
    let fundingAmount = 1000.0
    let user = Test.createAccount()

    let flowPrices = [1.00000000]
    let yieldPrices = [1.00000000]
    let expectedDebts = [615384.61538462]
    let expectedYieldUnits = [615384.61538462]
    let expectedCollaterals = [1000000.00000000]
    let actions: [String] = ["none"]
    ...
    logStep("Scenario6_EdgeCases_LargePosition", 0, actualDebt, expectedDebts[0], actualYieldUnits, expectedYieldUnits[0], actualCollateral, expectedCollaterals[0])
    let okDebt0 = equalAmounts(a: actualDebt, b: expectedDebts[0], tolerance: 0.0000001)
    let okY0 = equalAmounts(a: actualYieldUnits, b: expectedYieldUnits[0], tolerance: 0.0000001)
    let okC0 = equalAmounts(a: actualCollateral, b: expectedCollaterals[0], tolerance: 0.0000001)
    if !(okDebt0 && okY0 && okC0) { allGood = false }
    ...
    while i < flowPrices.length {
        ...
        logStep("Scenario6_EdgeCases_LargePosition", i, actualDebt, expectedDebts[i], actualYieldUnits, expectedYieldUnits[i], actualCollateral, expectedCollaterals[i])
        let okDebt = equalAmounts(a: actualDebt, b: expectedDebts[i], tolerance: 0.0000001)
        let okY = equalAmounts(a: actualYieldUnits, b: expectedYieldUnits[i], tolerance: 0.0000001)
        let okC = equalAmounts(a: actualCollateral, b: expectedCollaterals[i], tolerance: 0.0000001)
        if !(okDebt && okY && okC) { allGood = false }
