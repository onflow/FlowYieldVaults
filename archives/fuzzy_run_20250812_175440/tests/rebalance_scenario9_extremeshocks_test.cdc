...
access(all)
fun test_RebalanceTideScenario9_ExtremeShocks_FlashCrash() {
    let fundingAmount = 1000.0
    let user = Test.createAccount()

    let flowPrices = [1.00000000, 0.30000000]
    let yieldPrices = [1.00000000, 1.00000000]
    let expectedDebts = [615.38461539, 184.61538462]
    let expectedYieldUnits = [615.38461539, 184.61538462]
    let expectedCollaterals = [1000.00000000, 300.00000000]
    let actions: [String] = ["none", "Repay 430.769230770"]
    ...
    logStep("Scenario9_ExtremeShocks_FlashCrash", 0, actualDebt, expectedDebts[0], actualYieldUnits, expectedYieldUnits[0], actualCollateral, expectedCollaterals[0])
    ...
    while i < flowPrices.length {
        ...
        logStep("Scenario9_ExtremeShocks_FlashCrash", i, actualDebt, expectedDebts[i], actualYieldUnits, expectedYieldUnits[i], actualCollateral, expectedCollaterals[i])
...
access(all)
fun test_RebalanceTideScenario9_ExtremeShocks_Rebound() {
    let fundingAmount = 1000.0
    let user = Test.createAccount()

    let flowPrices = [0.30000000, 4.00000000]
    let yieldPrices = [1.00000000, 1.00000000]
    let expectedDebts = [184.61538462, 2461.53846154]
    let expectedYieldUnits = [184.61538462, 2461.53846154]
    let expectedCollaterals = [300.00000000, 4000.00000000]
    let actions: [String] = ["Repay 430.769230770", "Borrow 2276.923076923"]
    ...
    logStep("Scenario9_ExtremeShocks_Rebound", 0, actualDebt, expectedDebts[0], actualYieldUnits, expectedYieldUnits[0], actualCollateral, expectedCollaterals[0])
    Test.assert(equalAmounts(a: actualDebt, b: expectedDebts[0], tolerance: 0.0000001), message: "Debt mismatch at step 0")
    Test.assert(equalAmounts(a: actualYieldUnits, b: expectedYieldUnits[0], tolerance: 0.0000001), message: "Yield mismatch at step 0")
    Test.assert(equalAmounts(a: actualCollateral, b: expectedCollaterals[0], tolerance: 0.0000001), message: "Collateral mismatch at step 0")
    ...
    while i < flowPrices.length {
        ...
        logStep("Scenario9_ExtremeShocks_Rebound", i, actualDebt, expectedDebts[i], actualYieldUnits, expectedYieldUnits[i], actualCollateral, expectedCollaterals[i])
        Test.assert(equalAmounts(a: actualDebt, b: expectedDebts[i], tolerance: 0.0000001), message: "Debt mismatch at step \(i)")
        Test.assert(equalAmounts(a: actualYieldUnits, b: expectedYieldUnits[i], tolerance: 0.0000001), message: "Yield mismatch at step \(i)")
        Test.assert(equalAmounts(a: actualCollateral, b: expectedCollaterals[i], tolerance: 0.0000001), message: "Collateral mismatch at step \(i)")
        i = i + 1
...
access(all)
fun test_RebalanceTideScenario9_ExtremeShocks_YieldHyperInflate() {
    let fundingAmount = 1000.0
    let user = Test.createAccount()

    let flowPrices = [1.00000000, 1.00000000]
    let yieldPrices = [1.00000000, 5.00000000]
    let expectedDebts = [615.38461539, 2130.17751479]
    let expectedYieldUnits = [615.38461539, 426.03550296]
    let expectedCollaterals = [1000.00000000, 3461.53846154]
    let actions: [String] = ["none", "Bal sell 492.307692308 | Borrow 1514.792899409"]
    ...
    logStep("Scenario9_ExtremeShocks_YieldHyperInflate", 0, actualDebt, expectedDebts[0], actualYieldUnits, expectedYieldUnits[0], actualCollateral, expectedCollaterals[0])
    Test.assert(equalAmounts(a: actualDebt, b: expectedDebts[0], tolerance: 0.0000001), message: "Debt mismatch at step 0")
    Test.assert(equalAmounts(a: actualYieldUnits, b: expectedYieldUnits[0], tolerance: 0.0000001), message: "Yield mismatch at step 0")
    Test.assert(equalAmounts(a: actualCollateral, b: expectedCollaterals[0], tolerance: 0.0000001), message: "Collateral mismatch at step 0")
    ...
    while i < flowPrices.length {
        ...
        logStep("Scenario9_ExtremeShocks_YieldHyperInflate", i, actualDebt, expectedDebts[i], actualYieldUnits, expectedYieldUnits[i], actualCollateral, expectedCollaterals[i])
        Test.assert(equalAmounts(a: actualDebt, b: expectedDebts[i], tolerance: 0.0000001), message: "Debt mismatch at step \(i)")
        Test.assert(equalAmounts(a: actualYieldUnits, b: expectedYieldUnits[i], tolerance: 0.0000001), message: "Yield mismatch at step \(i)")
        Test.assert(equalAmounts(a: actualCollateral, b: expectedCollaterals[i], tolerance: 0.0000001), message: "Collateral mismatch at step \(i)")
        i = i + 1
...
access(all)
fun test_RebalanceTideScenario9_ExtremeShocks_MixedShock() {
    let fundingAmount = 1000.0
    let user = Test.createAccount()

    let flowPrices = [0.60000000, 0.40000000]
    let yieldPrices = [1.00000000, 2.20000000]
    let expectedDebts = [369.23076923, 518.81656805]
    let expectedYieldUnits = [369.23076923, 235.82571275]
    let expectedCollaterals = [600.00000000, 843.07692308]
    let actions: [String] = ["Repay 246.153846154", "Bal sell 201.398601399 | Borrow 149.585798816"]
    ...
    logStep("Scenario9_ExtremeShocks_MixedShock", 0, actualDebt, expectedDebts[0], actualYieldUnits, expectedYieldUnits[0], actualCollateral, expectedCollaterals[0])
    Test.assert(equalAmounts(a: actualDebt, b: expectedDebts[0], tolerance: 0.0000001), message: "Debt mismatch at step 0")
    Test.assert(equalAmounts(a: actualYieldUnits, b: expectedYieldUnits[0], tolerance: 0.0000001), message: "Yield mismatch at step 0")
    Test.assert(equalAmounts(a: actualCollateral, b: expectedCollaterals[0], tolerance: 0.0000001), message: "Collateral mismatch at step 0")
    ...
    while i < flowPrices.length {
        ...
        logStep("Scenario9_ExtremeShocks_MixedShock", i, actualDebt, expectedDebts[i], actualYieldUnits, expectedYieldUnits[i], actualCollateral, expectedCollaterals[i])
        Test.assert(equalAmounts(a: actualDebt, b: expectedDebts[i], tolerance: 0.0000001), message: "Debt mismatch at step \(i)")
        Test.assert(equalAmounts(a: actualYieldUnits, b: expectedYieldUnits[i], tolerance: 0.0000001), message: "Yield mismatch at step \(i)")
        Test.assert(equalAmounts(a: actualCollateral, b: expectedCollaterals[i], tolerance: 0.0000001), message: "Collateral mismatch at step \(i)")
        i = i + 1
